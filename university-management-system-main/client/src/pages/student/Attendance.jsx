...
export default function Attendance() {
  ...
  const [selectedCourse, setSelectedCourse] = useState(""); // Initialize with empty string instead of null
  ...
  useEffect(() => {
    async function fetchCourseAndExamTypeNames() {
      try {
        ...
        setCourses(resData?.courses || []); // Ensure we have at least an empty array
      } catch (error) {
        console.log(error);
      } finally {
        setIsLoading(false);
  ...
  async function handleFetchAttendances(courseId) {
    if (!courseId) return; // Don't fetch if courseId is empty
