...
const getAcademics = async (req, res) => {
  try {
    ...
    console.log(`Fetching academics for student: ${studentId}, course: ${courseId}, examType: ${examType}`);
    ...
    const academics = await academicSchema
      ...
      .select("activityNumber activityName weightage totalMarks marks");

    console.log(`Found ${academics.length} academic records`);
    ...
    if (academics.length) {
      ...
      for (let i = 0; i < academics.length; i++) {
        ...
        if (
          ...
          academicDetails.push({
            ...
            passingMarks: courseDetails.passingMarks || 50,
            maxMarks: courseDetails.maxMarks || 100,
            comment: requiredStudentMarks.comment || "",
            activityName: element.activityName || `${examType} ${element.activityNumber}`
          });
        }
      }
      
      console.log(`Returning ${academicDetails.length} academic details after filtering for visibility`);
      ...
      if (academicDetails.length) {
        ...
        res.status(404).send({
          success: false,
          message: "No visible academic records found for this student.",
        });
      }
    } else {
      res.status(404).send({
